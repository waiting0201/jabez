...
- **PDF 中文字型為 subset**（`Admin/src/assets/fonts/NotoSansTC-*.subset.ttf`，由 `Admin/scripts/generate-charset.py` + `npm run subset-fonts` 產生）：字集**必須含 Big5 第一 + 第二字面**（約 13,000 字）。2026-09 修正——原本只收第一字面，而**姓名罕用字多在第二字面**（例「闓」U+95D3），缺字時 jsPDF **不報錯、不印方框，是整個字消失**（「劉闓毅」印成「劉毅」），畫面與程式皆看不出異常。改字集後須重跑子集化並把 `.subset.ttf` 一起進版控，見 [docs/frontend-design.md §8.8](docs/frontend-design.md)
