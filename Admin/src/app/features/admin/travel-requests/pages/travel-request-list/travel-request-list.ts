...
export class TravelRequestList {
  ...
  canWrite()  { return this.auth.hasPermission('travel-requests:write'); }
  canDelete() { return this.auth.hasPermission('travel-requests:delete'); }
  ...
  /** 分期撥款三態 badge（核准後才顯示）*/
  installmentStatusOf(r: TravelRequest): PaymentInstallmentStatus | null {
    if (r.approvalStatus !== 'approved') return null;
    return r.paymentStatus ?? null;
  }

  delete(r: TravelRequest) {
    if (confirm(`確定要刪除此出差預支申請嗎？`)) {
