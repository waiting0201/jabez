...
} from '../../models/pre-review-request.model';
import {PagedResult} from '../../../../../shared/models/paged-result.model';
import {AuthService} from '@core/auth/services/auth.service';
import {HasPermissionDirective} from '@shared/directives/has-permission.directive';

@Component({
  selector: 'app-pre-review-list',
  templateUrl: './pre-review-list.html',
  imports: [RouterLink, DecimalPipe, DatePipe, HasPermissionDirective],
})
export class PreReviewList {
  private service = inject(PreReviewRequestService);
  private auth    = inject(AuthService);

  canWrite()  { return this.auth.hasPermission('pre-review-requests:write'); }
  canDelete() { return this.auth.hasPermission('pre-review-requests:delete'); }
  ...
  private result = toSignal(
    ...
    {initialValue: {items: [], totalCount: 0, page: 1, pageSize: 20, totalPages: 1} as PagedResult<PreReviewRequest>}
  );
  ...
  delete(r: PreReviewRequest) {
    if (confirm('確定要刪除此預審申請嗎？')) {
