...
@Component({
  ...
  templateUrl: './travel-payment-list.html',
  imports: [RouterLink, DatePipe, DecimalPipe, HasPermissionDirective],
})
export class TravelPaymentList {
  private service = inject(TravelPaymentRequestService);
  private auth = inject(AuthService);

  canWrite()  { return this.auth.hasPermission('travel-payment-requests:write'); }
  canDelete() { return this.auth.hasPermission('travel-payment-requests:delete'); }
  ...
  private result = toSignal(
    ...
    {initialValue: {items: [], totalCount: 0, page: 1, pageSize: 20, totalPages: 1} as PagedResult<TravelPaymentRequest>}
  );
