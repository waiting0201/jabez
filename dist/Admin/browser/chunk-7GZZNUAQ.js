...
var ApprovalTaskService = class _ApprovalTaskService {
  ...
  review(id, applicationType, action, reviewNote, estimatedPaymentDate, paidAt) {
    return this.http.patch(`${environment.apiUrl}/approval-tasks/${applicationType}/${id}/review`, { action, reviewNote, applicationType, estimatedPaymentDate, paidAt }).pipe(switchMap((updated) => this.getAll().pipe(map(() => updated))));
  }
...
var ngbRunTransition = (zone, element, startFn, options) => {
  ...
  zone.runOutsideAngular(() => {
    ...
    const timer$ = timer(transitionDurationMs + environment2.getTransitionTimerDelayMs()).pipe(takeUntil(stop$));
...
//# sourceMappingURL=chunk-7GZZNUAQ.js.map
