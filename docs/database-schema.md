...
| `PaymentRequest` | 請款申請（含 `RequestNo` 單號 `PR-yyyyMMdd-NNN`，**nullable + filtered unique index**（草稿為 NULL，送簽時取號）、`VendorId` nullable FK：當 Type=`vendor` 時必填且必須是 IsActive=true 的廠商；其他類型強制為 null；撥款資料統一由 `PaymentRequestInstallment[]` 表達，父表無 cache 欄位） |
| `InvoiceItem` | 請款明細（發票項目） |
| `PaymentRequestAttachment` | 一般請款整單批次附件（照片 / PDF，FK Cascade，存 `request-attachments`） |
| `PreReviewRequest` | 預審申請（事前預審：實際花費前送類似請款的單據走簽核，含報價單 / 品項 / 金額；含 `RequestNo` 單號 `PRV-yyyyMMdd-NNN`，**nullable + filtered unique index**（草稿為 NULL，送簽時取號）；**無撥款流程、金額不計入款項統計報表**） |
...
| `TravelPaymentRequest` | 出差請款申請（含 `RequestNo` 單號 `TPR-yyyyMMdd-NNN`，**nullable + filtered unique index**（草稿為 NULL，送簽時取號）；員工代墊後直接請款，無沖銷流程；撥款資料統一由 `TravelPaymentRequestInstallment[]` 表達，父表無 cache 欄位） |
...
| `AdvanceRequest` | 預支申請（含 `EstimatedRefundDate / RefundedAt / RefundedByUserId` 退款欄位、`RefundAmount / RefundedAmount` 退款金額；撥款資料統一由 `AdvanceRequestInstallment[]` 表達，父表無撥款 cache 欄位；`CurrentRoundNo` 最新預支批次號，> 1 且狀態為 pending/returned 表示有進行中的追加；`AdvanceNeededDate` **預支款需求日**（2026-09 新增，欄位 nullable、申請時必填）＝ Round 1 的需求日，Round ≥2 存於 `AdvanceRequestSupplement`） |
...
| `WriteOffRecord` | 預支沖銷申請（獨立簽核流程，關聯 AdvanceRequest，含 ApprovalStatus/CurrentStepOrder；`PendingClose` = 財務已登記結案，待整張單核准才寫 AdvanceRequest.IsClosed，退回/拒絕時清除；`RequestNo` **nullable + filtered 唯一索引**（草稿為 NULL，送簽時取號；2026-08 補上 EF 宣告擋併發，2026-09 改 filtered 並移除 `HasDefaultValue("")`）） |
...
| `TravelWriteOffRecord` | 出差預支沖銷申請（獨立簽核流程，關聯 TravelRequest；`PendingClose` 語意同 WriteOffRecord，對應 TravelRequest.IsClosed；`RequestNo` **nullable + filtered 唯一索引**（草稿為 NULL，送簽時取號；2026-08 新增時 migration 內含空白／重複單號清洗，2026-09 改 filtered 並移除 `HasDefaultValue("")`）） |
