...
| Method | Path | 說明 |
|--------|------|------|
| GET | `/approval-tasks` | 任務列表。`status`：`pending`（待審核）/ `approved`（已核准）/ `returned`（退回修改中）/ `rejected`（已拒絕）—— **走白名單正規化**，未帶＝維持既有行為（Superadmin 看全部非草稿、一般人看待審），帶了非法值一律當 `pending`。`scope=director`（總監室簽核範圍，與 `status` 四值自由組合）：`status=pending` ＝ 已輪到總監關卡（`CurrentStepOrder` 對應步驟 `JobTitle.Level=1`），其餘三態 ＝ 流程含總監關卡 + 對應 `ApprovalStatus`；僅財務管理部 / 會計室或 Superadmin 可查，其他部門回 403；財務管理部 / Superadmin 看全部，會計室只看流程中含會計室關卡的單。**舊值 `status=director_pending` 相容為 `scope=director&status=pending`**。另可帶 `paymentStatus`（撥款三態 `unpaid` / `partial` / `paid`，另加 `closed`＝已結案，只有預支 / 出差預支有結案概念、其餘類型不回傳）、`applicationType`（申請類型）、`submittedByUserId`（申請人；**僅財務體系部門或 Superadmin 生效**，其他人帶了一律忽略）、**`dateFrom` / `dateTo`**（申請日期區間，`YYYY-MM-DD`，**所有人可用、各頁籤常駐**；基準＝**送簽日 `SubmittedAt`**（舊資料退回 `CreatedAt`，與清單顯示同一值），`dateTo` **含當日**（SQL `< DATEADD(day, 1, @DateTo)`）；可單邊帶；解析失敗一律忽略；起迄顛倒不報錯、自然查無資料）、**`directorReviewedOn`**（總監簽核日，`YYYY-MM-DD`，**單一日期非區間**；**僅 `scope=director` 且 `status=approved` 生效**，其他組合一律忽略；比對「總監關卡（`JobTitle.Level=1`）對應 `StepOrder` 的 `ApprovalRecords` 且 `Action='approved'`」的 `ReviewedAt`，**含當日**（`>= @X AND < DATEADD(day,1,@X)`）；**綁關卡 StepOrder 而非簽核者職稱**，以涵蓋升級指派 / 代簽；一單多筆總監核准紀錄（追加預支 RoundNo / 退回重簽 / 多個 Level=1 關卡）取 EXISTS、不取 MAX；解析失敗一律忽略） |
| GET | `/approval-tasks/applicants` | 申請人下拉選項（**在職員工 ∪ 曾送出非草稿申請者**，去重後依姓名排序）；**僅財務體系部門或 Superadmin**，其他人回 403 |
| GET | `/approval-tasks/{id}` | 取得任務詳情 |
| GET | `/approval-tasks/{appType}/{id}` | 取得指定類型的任務詳情。存取控制：Superadmin / 有 `approval-tasks:read` / 被指定為審核者 / 曾審核過 / **申請人本人**（詳情頁簽核歷程與 PDF 簽名章皆取自此端點），其餘 403。回應含 **`stepReviewers`**（**逐關**列出實際可簽核的人 `{stepOrder, reviewers[]}`，僅 `status=pending` 計算；某關 `reviewers` 為空＝該關查無可簽核人員，單子會卡在那關）。⚠️ **`overtimeDetail` 刻意不回 `overtimePayAmount`**（2026-09）：改回 `hourTiers[]`（`{multiplier, hours}`，由 `payableHours` + 日別純函式導出），避免金額 ÷ 時數反推底薪 |
...
| GET | `/leave-requests/working-days?start=&end=&leaveType=` | 計算扣除國定假日與六日後的實際請假日清單與天數（**彈性休假日（原「補假」）不列入扣除**，雖為 `IsHoliday=1` 但要休得自己請假，見 [leave-rules.md §彈性休假日](business/leave-rules.md)；工作日型假別才扣假日＝除歲時祭儀假外的 16 種；任何登入者可呼叫，免 `calendar-days:read`；回 `hasCalendarData / holidayDates[] / workingDates[] / workingDays`） |
...
| POST | `/attendances/clock-in` | `attendances:write` | 上班打卡（含 GPS；落在已核准請假 `[StartDate, EndDate)` 區間內會回 BadRequest）。**四週彈性工時切換後**另加：依當日日別鎖定（例假日全鎖／休假日鎖／國定假日僅活動日預定人力解鎖）、08:30 前不開放（請上午半天假者不受此限）、超過 09:30 記 `IsLate`（**出差當日不判定**） |
...
| GET | `/reports/shift-schedule?year=&month=[&departmentId=]` | `reports-shift-schedule:read` | **〈出勤／排休總覽表〉**（四週彈性工時功能 B）：一人一列 × 當月每日一欄，回 `rows[].cells[]`（`dayType` ＋ `isActivityDay` / `isActivityAssignee` 疊加旗標）、每列配額統計與 `quotaSatisfied`，以及表尾 `dayStats[]`（每日 `workingCount` ＋ `noCoverage`）。**不分頁**（一次回完整表格才畫得出橫向月曆，匯出由前端用已載入資料產 Excel，同 payroll 做法）。可見範圍與 §3.1 同一套：持 `shift-schedule:view-all` → 全公司，否則受 `ProjectAccessScope` 限制。⚠ `noCoverage` 只在**週一至週五**且非國定假日時成立 —— 週末與國定假日本來就可能全員休假 |
