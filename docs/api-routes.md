...
| Method | Path | 說明 |
|--------|------|------|
| GET | `/approval-tasks` | 任務列表；`status` 參數：`pending`（待審核）/ `approved`（已核准）/ `rejected`（已拒絕）/ `director_pending`（總監待簽核，僅財務管理部或 Superadmin 可查，非財務管理部呼叫回 403）；另可帶 `paymentStatus`（撥款三態）、`applicationType`（申請類型）、`submittedByUserId`（申請人；**僅財務體系部門或 Superadmin 生效**，其他人帶了一律忽略） |
