...
| GET | `/approval-tasks/{appType}/{id}` | 取得指定類型的任務詳情。存取控制：Superadmin / 有 `approval-tasks:read` / 被指定為審核者 / 曾審核過 / **申請人本人**（詳情頁簽核歷程與 PDF 簽名章皆取自此端點），其餘 403。回應含 **`stepReviewers`**（**逐關**列出實際可簽核的人 `{stepOrder, reviewers[]}`，僅 `status=pending` 計算；某關 `reviewers` 為空＝該關查無可簽核人員，單子會卡在那關）|
