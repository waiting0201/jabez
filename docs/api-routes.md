...
| GET | `/approval-tasks/{appType}/{id}` | 取得指定類型的任務詳情。存取控制：Superadmin / 有 `approval-tasks:read` / 被指定為審核者 / 曾審核過 / **申請人本人**（詳情頁簽核歷程與 PDF 簽名章皆取自此端點），其餘 403。回應含 **`currentStepReviewers`**（目前關卡實際可簽核的人，僅 `status=pending` 計算；空陣列＝這關查無可簽核人員，單子卡住）|
