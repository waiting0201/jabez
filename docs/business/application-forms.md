...
## 日期欄位的年份防呆（2026-09 新增）

所有**由使用者填寫**的日期欄位（加班日期、請假起迄、出差起迄、預支日期／需求日、
發票日期、品項日期、子女出生日期）都限制在 **今日 ±3 年**；子女出生日期另限「過去 3 年內且不得晚於今日」。

- 前端：日期欄位掛 `min` / `max`（行動裝置的原生選擇器直接轉不到範圍外的年份），超出範圍給紅框
- 後端：`RequestDateGuard` 於 Create / Update 一律再驗一次，超出回 400，訊息會直接點名「是否誤填民國年」
- 範圍**是防呆不是業務規則**：期間長短、額度、發票期限等限制仍由各申請表原本的驗證負責

> 起因：2026-09 一張加班單的加班日期被打成民國年（`115/09/06` → 西元 0115 年）。
> 打卡頁以「加班日期 = 今日」撈已核准加班單，這張撈不到，當事人整天無法打加班卡；
> 而單子已核准（只有草稿能編輯），本人無法自行修正，最後只能直接改資料庫。
> iOS 日期選擇器對 1582 年前的日期改用儒略曆呈現，同一筆資料在清單頁顯示 `0115-09-06`、
> 編輯頁卻顯示「115年9月7日」，差一天，讓誤植更難被辨認。
...
| 參與人員個人參與日期 | —（不使用參與人員） | ✓ 每位參與人員可逐日勾選參與日期（可不連續，限活動期間內），**每個勾選日可再指定「全天 / 上半天 / 下半天」**；**未勾選＝全程參與**；個人假日津貼天數 = Σ(勾選日 ∩ 行事曆假日 的時段權重，全天 1.0 / 上下半天各 0.5)，Submit 時快照至 `TravelRequestParticipant.HolidayDays`（`decimal(5,1)`），未勾選者沿用整單 `HolidayDays`；**申請人不會自動列入（2026-09 改）**：要領假日津貼須自行加入清單，加入後比照一般參與者（可逐日、可半天）。同一人不可重複列入（DB 唯一索引 `(TravelRequestId, UserId)`，後端回 400、前端下拉排除已選過的人）|
| 簽核頁參與人員明細 | —（不使用參與人員） | ✓ 簽核作業詳情頁（`approval-task-review`）於「假日執行活動申請資訊」下方多一張 **參與執行人員** 卡：一人一列，含 人員（申請人若列於清單中掛 badge）/ 參與日期（`9/5、9/6 上午`，未逐日勾選顯示「全程參與」）/ 假日天數（半天顯示 `1.5`）；**卡片一律顯示**，清單為空時以 `@empty` 列出「尚無參與執行人員，本單不計入假日津貼」（申請詳情頁同）——2026-09 申請人不再自動占一列後，整張卡消失會讓審核者誤以為資料漏帶；**津貼只在表尾顯示合計，不列個人津貼**（2026-09 收斂：個人津貼 ÷ 天數＝該員日薪，等同揭露月薪；後端 `HolidayAllowanceDto` 已無 `Allowance` 欄，合計改由 `TravelTaskDetailDto.HolidayAllowanceTotal` 帶回，待審核清單摘要亦只顯示每人天數 + 全單合計）；資料走 `TravelTaskDetailDto.HolidayAllowances[].Dates`，日期格式化共用前端 `formatParticipantDates()`（與申請詳情頁同一真相）|
