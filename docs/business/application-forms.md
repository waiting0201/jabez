...
> **「申請日期」＝送出（送簽）那一天，不是建立草稿那一天**（2026-09 變更）。

10 種申請表的父表都有 `SubmittedAt`（`DateTime?`），與單號同一時機、同一組守則寫入：

| 時機 | 行為 |
|------|------|
| 建立草稿 | **不寫**，`SubmittedAt = NULL`，清單顯示「—」、詳情顯示「（送簽後產生）」 |
| 首次送簽 | 寫入送簽當下時間（`x.SubmittedAt ??= Clock.Now;`） |
| 退回後重送 | **不更新**，維持首次送簽日（與單號規則一致，兩個日期永遠對得起來） |
| 追加預支批次 | 父單 `SubmittedAt` 不變（批次本身另有自己的 `CreatedAt`） |
| Superadmin 送簽（自動核准） | 照常寫入 |

父表原有的 `CreatedAt` **維持原義**（建立草稿的時間），只是不再被當成申請日期使用。
全站「申請日期」的顯示與計算一律讀 `SubmittedAt`：各申請清單頁與詳情頁、簽核作業清單與詳情頁、
7 種列印 PDF 的申請日期／簽名欄日期、款項統計報表的欄位**與日期區間篩選**皆同一基準。

改版時已將所有非草稿的既有單回填 `SubmittedAt = CreatedAt`（唯一取得的到的近似值），
故歷史單據的顯示與報表數字不受影響；草稿刻意留 `NULL`。

## 一般申請表（5 種）

| # | 申請表 | 前端路徑 | API Prefix / RequestType | 自審分組 | 流程特性 |
|---|--------|----------|--------------------------|---------|---------|
| 1 | 請款申請 | `/admin/payment-requests` | `/payment-requests` / `payment_request` | **Group B 首位跳過** | 一般費用請款（含發票明細）；走簽核 + 撥款。**Type=`vendor` 時必須選擇 `Vendor` 主檔（廠商管理 `/admin/vendors`），找不到時可從表單即時新增**；**Type=`general`（一般請款）/ `other`（其他，2026-09 新增）皆不需選廠商，明細下方可批次上傳整單附件（照片 / PDF）**；**請款原因（`Reason`）必填**（前端 `Validators.required` + 後端 Create / Update 皆擋空白，草稿儲存亦需填寫） |
...
| 簽核頁參與人員明細 | —（不使用參與人員） | ✓ 簽核作業詳情頁（`approval-task-review`）於「假日執行活動申請資訊」下方多一張 **參與執行人員** 卡：一人一列，含 人員（申請人掛 badge）/ 參與日期（`9/5、9/6 上午`，未逐日勾選顯示「全程參與」）/ 假日天數（半天顯示 `1.5`）；**津貼只在表尾顯示合計，不列個人津貼**（2026-09 收斂：個人津貼 ÷ 天數＝該員日薪，等同揭露月薪；後端 `HolidayAllowanceDto` 已無 `Allowance` 欄，合計改由 `TravelTaskDetailDto.HolidayAllowanceTotal` 帶回，待審核清單摘要亦只顯示每人天數 + 全單合計）；資料走 `TravelTaskDetailDto.HolidayAllowances[].Dates`，日期格式化共用前端 `formatParticipantDates()`（與申請詳情頁同一真相）|
...
統一發票號碼固定為 **2 碼字軌（英文大寫字母）+ 8 碼數字**（如 `ED22598786`）。**收銀機 / 三聯式發票的字軌與 8 碼數字是分開印的**（字軌印在「中華民國 NNN 年 M-M 月份」之後，數字另在右側大字框內），LLM 容易只讀到其中一段、甚至改抄同一張上清晰的 8 碼純數字（買方「統編」欄或收銀機「NO.」機號）充當發票號碼。兩道防線：

1. **Prompt**（[InvoiceOcrHandler.cs](../../Api/Handlers/InvoiceOcrHandler.cs)）：明說字軌與數字常分開印須合併成 10 碼、**嚴禁**填 8 碼純數字（統編／機號都不是發票號碼）、字軌讀不出來時填 `""` 而非改抄他欄。
2. **後端後處理** `NormalizeInvoiceNo`：把「`ED 22598786`」「`ED-22598786`」合併為 10 碼；`docType === 'invoice'` 且 `invoiceNo` 為 8 碼純數字並**等於 `buyerTaxId` 或 `sellerTaxId`** 時清空該欄（確定抄錯欄位），讓前端顯示必填紅框由使用者補填 —— 寧可留白也不要帶入錯號存進 DB（發票號碼有唯一性檢查）。

> 高鐵票號的「移除 dash 保留 13 碼純數字」也在同一個 `NormalizeInvoiceNo` 內，順序上先處理，交通票根不受上述發票規則影響。

### 買方抬頭 / 統編驗證

OCR 同時辨識統一發票的**買方抬頭（買受人公司名稱）**、**買方統編**與**賣方統編**，上傳當下即時比對公司合法的 6 組「抬頭＋統編」白名單，不符者在**該列明細下方顯示紅字警告**（**非阻擋式，仍可送出**）。判定規則（前端共用工具 [`invoice-buyer-validator.ts`](../../Admin/src/app/shared/utils/invoice-buyer-validator.ts)，`VALID_INVOICE_BUYERS` 為單一真相）：
...
- **統編為主要錨點**（8 碼數字 OCR 較可靠；長中文公司名常被 OCR 缺字截斷，不宜硬比對）：
  - 統編符合某組 + 抬頭相容 → 通過。抬頭相容＝完全相等／互為子字串／**公司名前 3 個識別字相同**（容忍「雅比斯國際創意策略…」被 OCR 讀成「雅比斯…策略…」）／**同長度僅差 1 個字**（長度 ≥ 6 才適用）。差 1 字容錯與下方「統編差 1 碼」同一思路：統編是法定唯一識別碼且已完全命中白名單，長中文公司名只差 1 個字幾乎必為 OCR 形近字誤讀（實測「**雅**比斯…」被讀成「**羅**比斯…」）。白名單 6 組的抬頭彼此不會落入「同長度差 1 字」，不會互相誤放行。
  - 統編符合某組、但抬頭明顯為**他家公司**（前綴不同）→ 警告「買方抬頭「{抬頭}」與統編 {統編} 不符，請確認。」
  - 統編不在 6 組內、且**非 8 碼或檢查碼不合**（財政部 2023 新制：權重 `[1,2,1,2,1,2,4,1]` 逐位乘積拆位相加，總和須為 5 的倍數；第 7 碼為 7 時另允許總和 +1 為 5 的倍數）：
  ...
  - 統編不在 6 組內、但格式合法 → 警告「買方統編「{統編}」不在公司白名單，請確認。」
- **三種警告訊息一律帶出讀到的統編**，使用者才能自行判斷是 OCR 抓錯欄位、手寫誤讀，還是真的開錯抬頭。
- 正規化：統編全形轉半形後只留數字；抬頭移除空白後比對。
- **警告列右側附「確認無誤」checkbox**：容錯規則沒接住的誤判（OCR 讀成完全不同的公司名、統編抄到他欄等）由使用者自行放行，勾選後該列由紅字轉灰字並加註「已確認無誤：」。**勾選與否不影響送出**（本來就不阻擋），僅為讓申請人明確表態「這張我核對過了」；同一列重新 OCR 或刪列後確認狀態即失效。
- 警告與人工確認皆僅供上傳當下提示，**不持久化**（不存 DB、不寫 migration、不進 DTO）；重開草稿不會重現警告，審核者端亦看不到。

> 設計取捨：統一編號是法定唯一識別碼，正確即代表是本公司（含區分總公司 / 營業所）。實測 Gemini 對長公司名常缺字（如把「雅比斯國際創意策略股份有限公司」讀成「雅比斯策略股份有限公司」），故以統編為準、抬頭採寬鬆比對，避免每張正確發票都跳假警告。

> 歷史（2026-09）：一張收銀機三聯式發票（買方統編 `28830371`，雅比斯總公司，白名單內）被誤跳「抬頭與統編不符」。成因為 OCR 把「**雅**比斯國際創意策略股份有限公司」讀成「**羅**比斯…」，前 3 字比對不過。修正＝抬頭比對加「同長度差 1 字」容錯（治本）＋ 警告列加「確認無誤」checkbox 供人工放行（治標，涵蓋容錯沒接住的誤判）。
