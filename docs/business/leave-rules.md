...
| 半天 (`half_day`) | 4 小時 = 半天 | 日期 + 上午/下午 選擇（上午起點：**補休 09:00**、其餘 08:00） | 年假、補休、高階主管假 |
...
- **半天時段的代表性時刻**：`StartDate` / `EndDate` 存的是時段代表時間 —— 上午 `08:00–12:00`、下午 `13:00–17:00`；
  **唯獨補休的上午自 `09:00` 起**（`09:00–12:00`），時段下拉標籤與清單頁顯示的時間隨之改變。
  單一真相為前端 [`halfDayAmStartHour()`](../../Admin/src/app/features/admin/leave-requests/models/leave-request.model.ts)，
  送出 payload 與下拉標籤共用，勿另行硬編碼 08。
  > 這是**顯示與儲存時刻**的差異，**不改時數**：半天恆 4 小時、全日恆 8 小時。後端一律以
  > 「起 &lt; 13:00 ＝上午」分類時段（`LeaveDayExpander.ExpandHalfDayUnit`），**不可改用「等於 08:00」判定**；
  > 逐日展開仍取標準上午時段 08:00–12:00（故出缺勤報表的請假欄、應出勤時段、登入自動補卡皆不受影響）。
  > 半天時數本身走「HalfDay 信任 client」，`body.Hours` 未帶時的退路 `LeaveRequestHandler.ComputeHalfDaySlotHours`
  > 亦以時段分類推時數，不用 End − Start 時間差（09:00–12:00 的時間差是 3 小時，會被「4 的倍數」檢查誤擋）。
...
- 依系統統計之加班工時扣抵
- **請假時段**：半天單位，**上午時段自 09:00 起**（09:00–12:00）、下午 13:00–17:00；時數仍以半天 4 小時計（見上方「半天時段的代表性時刻」）。
- **可補休時數來源兩塊**：
