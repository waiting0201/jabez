...
| **整批不發（窗仍不夠長）** | 2026-09-08 的下班提醒無任何 `TargetTime=18:00` 的 batchStart，全體員工都沒收到 | 17:58–18:07 這 10 分鐘內一次 tick 都沒觸發到；同一時段有 9 人正常打下班卡，證明 App 是活的，純粹是排程漏 tick |

修正方式是把「準時」的責任從平台移到程式：

- **時間窗**（`WindowMinutes = 30`）吸收冷啟動延遲 —— 窗內任何一次 tick 都算命中。窗的尾端（09:28 / 18:28 Taipei）仍落在 cron 涵蓋的時段（台北 7-9 / 16-18 時）內，**放寬窗長不必動 cron**
  - 2026-09-09 由 10 → 30。放寬是安全的：收件人 SQL 本來就排除「今日已打該類型卡」的人，**晚發只會發給還沒打卡的人**，語意從「再 N 分鐘上班」自然轉為「你還沒打卡」
  - 配套：Flex Message 的「剩餘時間」改由 `AttendanceReminderService` 以推播當下的實際時間差算出（不再固定帶 `LeadMinutes`），目標時刻已過時文案改為「已過 N 分鐘」，避免 09:25 推播卻寫「再 2 分鐘上班」
  - 這仍是「賭 30 分鐘內有一次 tick」。要根治得讓 host 不 scale-to-zero（Flex Consumption 加 1 個 Always Ready 執行個體，約 US$20–30/月），成本評估後決定先走免費方案
