...
| 4 | `NotifyApplicantPaidAsync` | `[已撥款] 您的{label} #{id} 已撥款 — {amount} 元` | 申請人 | 財務設定 PaidAt 從 null → 有值 | `PaymentRequestHandler` / `AdvanceRequestHandler` / `TravelRequestHandler` / `TravelPaymentRequestHandler` |
...
| 8 | `NotifyFinanceTravelRefundAsync` | `[需匯款] 出差申請 #{id} 沖銷超額 — 差額 {金額} 元` | 財務部全員 | 出差沖銷核准且金額超過出差金額 | `ApprovalTaskHandler.CloseTravelRequestAsync` |
