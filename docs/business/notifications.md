...
- **判斷時點**：以 `WorkStartTime` / `WorkEndTime` 為準，提醒提前 2 分鐘；命中條件為**時間窗** `[提前 2 分, +30 分)`（吸收冷啟動延遲，2026-09-09 由 10 分鐘放寬，見 [attendance-reminder.md](attendance-reminder.md)），同一槽一天只發一次由 `batchStart` 冪等閘保證
